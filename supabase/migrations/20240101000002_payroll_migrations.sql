...
drop policy if exists "Employees view own salary" on employee_salary_history;
drop policy if exists "Employees view own salary" on employee_salary_history;
create policy "Employees view own salary" on employee_salary_history for select using (auth.uid() = employee_id);
drop policy if exists "Admins view all salaries" on employee_salary_history;
drop policy if exists "Admins view all salaries" on employee_salary_history;
create policy "Admins view all salaries" on employee_salary_history for select using (is_admin());
drop policy if exists "Admins insert salaries" on employee_salary_history;
create policy "Admins insert salaries" on employee_salary_history for insert with check (is_admin());
drop policy if exists "Admins update salaries" on employee_salary_history;
drop policy if exists "Admins update salaries" on employee_salary_history;
...
drop policy if exists "Employees view own summary" on monthly_attendance_summary;
drop policy if exists "Employees view own summary" on monthly_attendance_summary;
create policy "Employees view own summary" on monthly_attendance_summary for select using (auth.uid() = employee_id);
drop policy if exists "Admins view all summaries" on monthly_attendance_summary;
drop policy if exists "Admins view all summaries" on monthly_attendance_summary;
create policy "Admins view all summaries" on monthly_attendance_summary for select using (is_admin());
drop policy if exists "Admins insert summaries" on monthly_attendance_summary;
